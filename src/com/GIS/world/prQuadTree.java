...
public class prQuadTree<T extends Compare2D<? super T>> {
	 ...
	 * Returns an array of doubles bounding the desired quadrant.
	...
	private prQuadNode insertHelper(prQuadNode sRoot, T elem, double xLo, double xHi, double yLo, double yHi) {
		...
		// If sRoot is an internal node, we'll insert recursively.
			...
			// Find the direction of where we should place the node and get the bounds of that region.
		...
		// Else, we need to create a new internal node and insert the old one as a new leaf.
		else {
	...
	public boolean insert(T elem) {
		...
		// Reject insertion if it's a duplicate or if it's outside of the world's boundaries.
	...
	private T findHelper(T Elem, prQuadNode node, double xLo, double xHi, double yLo, double yHi) {
		...
		// Else, the node is a leaf and we should return the data element if it's equal to what we're searching for.
		else {
	 ...
	 */
	private boolean findOverlap(double[] firstRegionBounds, double secondRegionXLo, double secondRegionXHi, double secondRegionYLo, double secondRegionYHi) {
		...
		return (firstRegionBounds[0] + firstRegionWidth >= secondRegionXLo && firstRegionBounds[2] + firstRegionHeight >= secondRegionYLo
				&& firstRegionBounds[0] <= secondRegionXLo + secondRegionWidth && firstRegionBounds[2] <= secondRegionYLo + secondRegionHeight);
	}
	...
	private void findHelper(ArrayList<T> collection, prQuadNode node, long xLo, long xHi, long yLo, long yHi) {
		...
		// Check if the node is an internal node.
			...
			/**
			 * Find overlapping quadrants and check that their leaves are within the
			 * region.
			 */
			prQuadInternal internalNode = (prQuadInternal) node;
		...
		/**
		 * Else, we have a leaf node and if its element is in the region, we'll
